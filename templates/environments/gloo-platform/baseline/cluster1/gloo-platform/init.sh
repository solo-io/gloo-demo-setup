...
if [[ ${gloo_mesh_version} == "" ]]
  then
    ...
    echo "Please provide the gloo_mesh_version to use (i.e. ${GME_VERSION}):"
...
spec:
  ...
  source:
    ...
    repoURL: ${GME_HELM_REPO}
    targetRevision: ${GME_VERSION}
  syncPolicy:
    automated:
      prune: true
      selfHeal: true
      ...
      values: |
        ...
        telemetryCollector:
            ...
            image:
              ...
              repository: ${GME_DOCKER_REPO}/gloo-otel-collector
              tag: ${GME_VERSION}
            config:
                exporters:
                    otlp:
